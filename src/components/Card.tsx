...
export default function Card({ item }: { item: CardProps }) {
  ...
  for (let i = 0; i < Math.trunc(rate); i++) {
    stars.push(<Star key={`full-${i}`} className="fill-amber-300" strokeWidth={0} />);
  }
  for (let i = Math.trunc(rate); i < rate; i++) {
    stars.push(<StarHalf key={`half-${i}`} className="fill-amber-300" strokeWidth={0} />);
  }
  for (let i = rate; i < 5; i++) {
    if (stars.length < 5) stars.push(<Star key={`empty-${i}`} className="" strokeWidth={0} />);
  }
  ...
  return (
    <Link to={linkTo}>
      <div className="w-56 p-4 rounded-lg hover:scale-105 hover:cursor-pointer">
        <img src={imgSrc} className="w-full h-72 object-cover rounded-2xl " alt={title} />
